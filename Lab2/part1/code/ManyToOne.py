...
# Combines all subtopics into one text file

# Combines all subtopic into one text file
def ManyToOne(data_source, subtop1, subtop2, subtop3, subtop4, subtop5):
    with open("../data/" + data_source + "/" + data_source + ".txt","w") as data_file:
        with open("../data/"  + data_source + "/" + subtop1 + "_data_clean.txt","r") as esports:
            esports_data = esports.readlines()
            for data in esports_data:
                data_file.write(data)
            esports.close()
        with open("../data/"  + data_source + "/"  + subtop2 + "_data_clean.txt","r") as nba:
            nba_data = nba.readlines()
            for data in nba_data:
                data_file.write(data)
            nba.close()
        with open("../data/" + data_source + "/"  + subtop3 + "_data_clean.txt","r") as ncaam:
            ncaam_data = ncaam.readlines()
            for data in ncaam_data:
                data_file.write(data)
            ncaam.close()
        with open("../data/" + data_source + "/"  + subtop4 + "_data_clean.txt","r") as nfl:
            nfl_data = nfl.readlines()
            for data in nfl_data:
                data_file.write(data)
            nfl.close()
        with open("../data/" + data_source + "/"  + subtop5 +  "_data_clean.txt","r") as nhl:
